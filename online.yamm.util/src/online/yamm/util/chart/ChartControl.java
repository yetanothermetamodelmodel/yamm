...
//							Double lastXValue = seriesToUpdate.getXValues().get(seriesToUpdate.getYValues().size() - 1);
							...
							if (view != null)
							{
								view.setValues(simulationTab.getTabId(),
										((online.yamm.util.chartModel.chart.Chart) seriesToUpdate.eContainer())
												.getChartId(),
	...
	public void initViewAll()
	{
		if (view != null && chartModel != null)
		{
			for (Tab tab : chartModel.getTabs())
			{
				...
				for (online.yamm.util.chartModel.chart.Chart chart : tab.getCharts())
				{
		...
		try
		{
			...
			if (wasOpen)
			{
				if (!view.tabIsActive(simulationTab.getTabId()))
				{
					...
					for (online.yamm.util.chartModel.chart.Chart chart : simulationTab.getCharts())
					{
						view.addChart(tabId, chart.getChartId(), chart.getXTitle(), chart.getYTitle());
	...
	{
		online.yamm.util.chartModel.chart.Chart chart = ChartFactory.eINSTANCE.createChart();
